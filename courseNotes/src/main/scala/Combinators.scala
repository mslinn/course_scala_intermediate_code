...
object Combinators extends App {
  ...
  val map: immutable.Map[Int, String] = immutable.Map(1 -> "a", 2 -> "b", 3 -> "c", 4 -> "d")
  println(s"""map.map { nv => (nv._1 * 3, nv._2 * 3) } = ${ map.map { nv: (Int, String) => (nv._1 * 3, nv._2 * 3) } }""")
...
object FuncMeth extends App {
  class Klass {
    ...
    val function1: Int => String =
      (y: Int) => s"x=$x and y=$y from function 1"
  }
