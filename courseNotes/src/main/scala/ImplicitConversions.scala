...
object InnerScope {
  implicit val list2: List[Int] = List(1, 2, 3)

  def res(implicit list: List[Int]): List[Int] = list
}

object OuterScope extends App {
  implicit val list: List[Int] = List(1, 2)
...
object ImplicitCoercion extends App {
  ...
  println(s"""Complex(2.0, 5.0) + 6 = ${ Complex(2.0, 5.0)+ 6d }""")
  println(s"""Complex(2.0, 5) + 6f = ${ Complex(2.0, 5.0) + 6d }""")
